...
void Adafruit_ST7735::initR(uint8_t options) {
  // eggfly
  Serial.println("Adafruit_ST7735::initR");
  commonInit(Rcmd1);
  if(options == INITR_GREENTAB) {
    ...
    _colstart = 26; // eggfly mod from 24 to 26
    _rowstart = 1;  // eggfly should mod or not?
   } else {
  ...
  if((options == INITR_BLACKTAB) || (options == INITR_MINI160x80)) {
    uint8_t data = 0xC0;
    // eggfly
    Serial.println("Black tab, change MADCTL color filter");
...
void Adafruit_ST7735::setRotation(uint8_t m) {
  uint8_t madctl = 0;
  // eggfly
  Serial.println("Adafruit_ST7735::setRotation");
